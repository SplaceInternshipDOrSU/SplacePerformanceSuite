...
import {messageClear, peerRatingAdd,fetchMyPeer,fetchPeerRating} from "../../../store/Reducers/ratingReducer";
...
export default function EmployeePerformanceEvaluation() {
  ...
  const {successMessage, errorMessage, myPeer, peersRating} = useSelector(state=>state.rating)
...
const handleSubmit = (e) => {
  ...
  dispatch(peerRatingAdd( {evaluatedUser:myPeer._id, evaluator:userInfo._id, roleOfEvaluator: "peer", category: myPeer.category.slug, ratings:evaluationData, quarter, year}))
...
useEffect(()=>{
  dispatch(fetchPeerRating({evaluatorID:userInfo._id ,userId:myPeer._id, quarter, year}))
},[dispatch, myPeer,quarter, year, userInfo])
...
if (peersRating?.ratings) {
  peersRating.ratings.forEach(r => {
...
useEffect(()=>{
  dispatch(fetchMyPeer({userId:userId}))
},[userId, dispatch])
    ...
    <div className="pb-5 w-full flex-wrap  min-h-screen flex justify-center">
        <div className="grid md:grid-cols-12 ">
            ...
            >
                ...
                PEER RATING
               <span>{Object.keys(peersRating).length > 0 ? '(DONE)' : ''}</span>
                ...
                return (
                    ...
                    {
                      Object.keys(peersRating).length > 0 ? (
                         <div className="absolute inset-0 w-full h-full flex items-center justify-center">
                        {/* Background overlay */}
                        <div className="absolute inset-0 bg-accent opacity-30 z-0"></div>

                        {/* Foreground content */}
                        <h2 className="relative z-10 font-bold text-slate-900 text-xl">DONE</h2>
                      </div>
                ...
                <button
                    type="submit"
                    className={`bg-accent font-bold text-white px-10 py-3 rounded-md hover:bg-blue-700 transition ${Object.keys(peersRating).length > 0 ? 'hidden':''}`}
                >
                    Submit Evaluation
                </button>
                </div>
                  ...
                  <div className="rounded-full h-[200px] w-[200px] absolute -top-[80px] -left-[70px]">
                    ...
                    <div className="h-full w-full flex justify-between items-end flex-col pt-9  p-4 text-center text-xl text-slate-200 text-wrap">
                     <div className="w-9/12 ">
                       <h2 className="font-bold text-3xl text-wrap">{myPeer.name}</h2>
                       <div className="">
                        <p className="text-base uppercase font-bold">{myPeer.role?.name} ( <span>{myPeer.category?.name}</span> )</p>
                       </div>
                     </div>
                     ...
                     {criteria.map(({ id, title, maxScore}) => {
                        ...
                        const savedRating = peersRating?.ratings?.find(r => r.criterion === title);
                        // const savedRatingComments = peersRating?.ratings?.find(r => r.criterion === title);
